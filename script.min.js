...
function issueResultImageUrl(src) {
  return `${src}?v=${ISSUE_RESULT_CACHE}`;
}

function prefetchIssueResultImages(issueId) {
  const result = ISSUE_RESULTS[issueId];
  if (!result) return;
  [result.before, result.after].forEach((src) => {
    const img = new Image();
    img.decoding = "async";
    img.src = issueResultImageUrl(src);
  });
}

const ISSUE_RESULTS = {
  skin: {
    before: "./images/issues/issue-skin-before.jpg",
    after: "./images/issues/issue-skin-after.jpg",
    beforeCaption: "Itching and irritated skin",
    afterCaption: "Calm skin after care",
  },
  shedding: {
    before: "./images/issues/issue-shedding-before.jpg",
    after: "./images/issues/issue-shedding-after.jpg",
    beforeCaption: "Too much hair fall",
    afterCaption: "Fuller coat after care",
  },
  coat: {
    before: "./images/issues/issue-coat-before.jpg",
    after: "./images/issues/issue-coat-after.jpg",
    beforeCaption: "Dry, dull coat",
    afterCaption: "Soft shine after care",
  },
  vomiting: {
    before: "./images/issues/issue-hairball-before.jpg",
    after: "./images/issues/issue-hairball-after.jpg",
    beforeCaption: "Hairballs",
    afterCaption: "Settled tummy after care",
  },
  dental: {
    before: "./images/issues/issue-dental-before.jpg",
    after: "./images/issues/issue-dental-after.jpg",
    beforeCaption: "Bad breath",
    afterCaption: "Fresher mouth after care",
  },
  bathing: {
    before: "./images/issues/issue-bathing-before.jpg",
    after: "./images/issues/issue-bathing-after.jpg",
    beforeCaption: "Needs a bath",
    afterCaption: "Clean coat after a bath",
  },
  grooming: {
    before: "./images/issues/issue-grooming-before.jpg",
    after: "./images/issues/issue-grooming-after.jpg",
    beforeCaption: "Matted, ungroomed fur",
    afterCaption: "Neat coat after grooming",
  },
  dental_care: {
    before: "./images/issues/issue-dentalcare-before.jpg",
  ...
  chubby: {
    before: "./images/issues/issue-chubby-before.jpg",
    after: "./images/issues/issue-chubby-after.jpg",
    beforeCaption: "Getting chubby",
    afterCaption: "Healthier weight after care",
  },
  second_opinion: {
    before: "./images/issues/issue-opinion-before.jpg",
    after: "./images/issues/issue-opinion-after.jpg",
...
function bindYoungIssuePicker() {
  assflowMain.querySelectorAll(".young-issue-card").forEach((card) => {
    const issueId = card.dataset.symptomId;
    if (issueId && issueId !== "general_care") {
      card.addEventListener("pointerdown", () => prefetchIssueResultImages(issueId), {
        passive: true,
      });
    }
  });
  ...
  assflowMain.querySelectorAll('.young-issue-card input[name="young-symptoms"]').forEach((input) => {
    input.addEventListener("change", () => {
      ...
      prefetchIssueResultImages(symptom.id);
...
function renderYoungSymptomStep() {
  ...
  const schedulePrefetch = window.requestIdleCallback || ((cb) => window.setTimeout(cb, 200));
  schedulePrefetch(() => {
    issues.forEach((issue) => {
      if (issue.id !== "general_care") prefetchIssueResultImages(issue.id);
    });
  });
      ...
      <div class="young-result-ba" aria-label="Before and after">
        <figure class="young-result-card">
          <div class="young-result-photo">
            <img src="${issueResultImageUrl(result.before)}" alt="" width="540" height="405" fetchpriority="high" decoding="async" loading="eager" />
        ...
        <figure class="young-result-card">
          <div class="young-result-photo">
            <img src="${issueResultImageUrl(result.after)}" alt="" width="540" height="405" fetchpriority="high" decoding="async" loading="eager" />
