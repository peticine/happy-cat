...
const HERO_VARIANTS = {
  water: {
    ...
    lead: "A quick check, then a ₹49 consult with Dr. Ankita — usually within 15–30 minutes.",
  ...
  litter: {
    ...
    lead: "Answer one quick question — then book a ₹49 consult with Dr. Ankita (usually within 15–30 minutes).",
  ...
  senior: {
    ...
    lead: "A quick senior check, then a ₹49 consult with Dr. Ankita — usually within 15–30 minutes.",
 ...
 * Open hours → ASAP only (no slot picker).
 * Outside hours → next open day’s 30‑min slots.
 */
...
function getVetCallSchedule(now = new Date()) {
  ...
  const slots = buildVetCallHalfHourSlots()
...
function renderYoungConnectStep() {
  ...
  form?.addEventListener("submit", async (event) => {
    ...
    quizState.whatsappNumber = number;
    quizState.contactMethod = "call";
    quizState.vetCallSlot = schedule.availableNow ? null : selected;
    if (petName) {
